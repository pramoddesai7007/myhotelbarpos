...
const MenuReport = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get(
        `https://hotelbarvtwo.vercel.app/api/order/orders/date?startDate=${startDate}&endDate=${endDate}`
      );
   ...
   Today's Date: ${startDates}
    ...
    <div class="report-content">
      ...
      currency: "INR",
           ...
           <tr class="bg-gray-100 table_downward">
            <td class="label">Last Total:</td>
            <td class="value">${Math.round(
      calculateTotal("lastTotal")
    ).toLocaleString("en-IN", {
  ...
  const handleDetailedPrint = () => {
    const detailedPrintContent = orders.map((order, index) => ({
      ...
      grandTotal: Math.round(order.grandTotal).toLocaleString("en-IN", {

        currency: "INR",
      }),
      lastTotal: Math.round(order.lastTotal).toLocaleString("en-IN", {
 ...
 Today's Date: ${startDates}
      ...
      (detail) => `
                  <tr>
                    ...
                    <td>${Math.round(detail.grandTotal)}</td>
                     <td>${Math.round(detail.lastTotal)}</td>
              ...
              <td>₹${Math.round(calculateTotal("grandTotal"))}</td>
               <td>₹${Math.round(calculateTotal("lastTotal"))}</td>
            </tr>
            </tbody>
          </table>
        </div>
  ...
  const handleUpdateOrders = async () => {
    ...
    try {
        ...
        const response = await axios.put("https://hotelbarvtwo.vercel.app/api/order/orders/flag");
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody>
                ...
                <tr className="bg-gray-100">
                  ...
                  <td className="px-4 py-2 whitespace-no-wrap border-b border-gray-200 font-bold">
                    {Math.round(calculateTotal("grandTotal")).toLocaleString(
                      "en-IN",
                  ...
                  <td className="px-4 py-2 whitespace-no-wrap border-b border-gray-200 font-bold">
                    {Math.round(calculateTotal("lastTotal")).toLocaleString(
                      "en-IN",
                      { style: "currency", currency: "INR" }
                    )}
                  </td>

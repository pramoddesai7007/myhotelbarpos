...
const Coupon = () => {
    ...
    const fetchHotelInfo = async () => {
      try {
        ...
        const allHotelsResponse = await axios.get(
          "https://hotelbarvtwo.vercel.app/api/counterHotel/get-all"
        );
        ...
        if (defaultHotelId) {
          ...
          const response = await axios.get(
            `https://hotelbarvtwo.vercel.app/api/counterHotel/getHotel/${defaultHotelId}`
          );
  ...
  const handlePrintBill = async () => {
    try {
      const [couponResponse, counterResponse] = await Promise.all([
        axios.get("https://hotelbarvtwo.vercel.app/api/coupon/coupon/latest"),

        axios.get("https://hotelbarvtwo.vercel.app/api/counter"),
      ]);
  ...
  //       `https://hotelbarvtwo.vercel.app/api/counterHotel/getHotel/661e0d39092937c54273c154`
  ...
  //       "https://hotelbarvtwo.vercel.app/api/coupon/billCouponOrder",
  //       saveData
  //     );
  ...
  //       `https://hotelbarvtwo.vercel.app/api/coupon/orderNumber/${orderNumber}`
  //     );
  ...
  useEffect(() => {
    axios
      .get("https://hotelbarvtwo.vercel.app/api/counter")
      .then((response) => {
    ...
    axios
      .get("https://hotelbarvtwo.vercel.app/api/table/tables")
      .then((response) => {
  ...
  useEffect(() => {
    if (selectedCategory) {
      axios
        .get(`https://hotelbarvtwo.vercel.app/api/counter/${selectedCategory._id}`)
        .then((response) => {
  ...
  const printCoupon = async () => {
    try {
      ...
      const countersResponse = await axios.get(
        "https://hotelbarvtwo.vercel.app/api/counter"
      );
      ...
      const latestCouponResponse = await axios.get(
        "https://hotelbarvtwo.vercel.app/api/coupon/coupon/latest"
      );
  ...
  return (
    <>
      <div className=" font-sans mt-2">
        ...
        <div className="container mx-auto">
          <div className="flex lg:flex-row shadow-lg ">
            ...
            <div className="flex flex-row flex-wrap px-2 ml-1 whitespace-nowrap">
      <span
        key="all-items"
      ...
      {categories.slice(0, showMore ? categories.length : 17).map((category) => (
        <span
          key={category._id}
          className={`whitespace-nowrap flex flex-col justify-center items-center cursor-pointer ml-3 mb-4 rounded-2xl lg:text-lg md:text-sm text-xl sm:text-xs font-semibold ${selectedCategory === category ? "bg-yellow-500 text-white" : ""}`}
          onClick={() => handleCategoryClick(category)}
          style={{ 
            height: '64px', 
            width: '100px', 

...
const AdminSettings = () => {
  ...
  //   try {
  //     const response = await axios.get('https://hotelbarvtwo.vercel.app/api/auth/admins');
  ...
  const handleToggleChange = async (adminId, currentStatus) => {
    try {
      ...
      const response = await axios.patch(`https://hotelbarvtwo.vercel.app/api/auth/admin/${adminId}/direct`, {
        isDirect: newValue
      });

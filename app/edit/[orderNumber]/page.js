...
const EditOrderPage = ({ params, tableId }) => {
      ...
      try {
        if (orderNumber) {
          ...
          const orderResponse = await axios.get(
            `https://hotelbarvtwo.vercel.app/api/order/get/order/${orderNumber}`
          );
          ...
          const sectionResponse = await axios.get(
            `https://hotelbarvtwo.vercel.app/api/section/sectionlist/${tableId}`
          );
  ...
  const handleSaveBill = async () => {
    try {
      ...
      await axios.post(`https://hotelbarvtwo.vercel.app/api/logHistory/log-history`, {
      ...
      if (itemsForStockIn.length > 0) {
        ...
        await axios.post(
          "https://hotelbarvtwo.vercel.app/api/liquorBrand/liquorBrand/stockIn",
          {
      ...
      const tableResponse = await axios.get(
        `https://hotelbarvtwo.vercel.app/api/table/tables/${tableId}`
      );
      const tableInfo = tableResponse.data;

      const updateOrderEndpoint = `https://hotelbarvtwo.vercel.app/api/order/update-order-by-number/${orderNumber}`;
...
<div class="datas">
  ...
  ${
    acPercentage > 0 && calculateTotal(currentOrder).acPercentageAmount > 0
      ? `
      <div style="display: flex; justify-content: space-between; margin-bottom: 7px; margin-top: 3px; font-size: 11px;">
        <span>AC Amount</span>
...
`;
  ...
  const handleOnlineBill = async () => {
   ...
   await axios.patch(`https://hotelbarvtwo.vercel.app/api/order/update-order-by-id/${orderIdToUpdate}`, {
  ...
  useEffect(() => {
    axios
      .get("https://hotelbarvtwo.vercel.app/api/main")
      .then((response) => {
        console.log(response.data);
  ...
  const calculateTotal = () => {
    ...
    const grandTotal =
      menuTotal + total + acPercentageAmount + CGST + SGST + VAT;

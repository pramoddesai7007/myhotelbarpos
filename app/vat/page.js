...
const VATForm = ({ onSubmit }) => {
    ...
    useEffect(() => {
        const fetchVATPercentage = async () => {
            try {
                if (hotel._id) {
                    const vatResponse = await axios.get(`https://hotelbarvtwo.vercel.app/api/hotel/get/${hotel._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (hotel._id && vatPercentage !== '') {
            try {
                await axios.patch(`https://hotelbarvtwo.vercel.app/api/hotel/vat/${hotel._id}`, {

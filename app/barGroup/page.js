...
const BarGroupMenu = () => {
  ...
  const handleAddMenusToCategory = async () => {
    try {
      ...
      const response = await axios.post(
        `https://hotelbarvtwo.vercel.app/api/liquorBrand/${selectedMainCategory}/assignmenus`,
        {
  ...
  const fetchAllMenus = async () => {
    try {
      const response = await axios.get(
        "https://hotelbarvtwo.vercel.app/api/liquorBrand/barSubmenu/list"
      );
      console.log(response.data);
    ...
    const fetchData = async () => {
      try {
        ...
        const mainCategoriesResponse = await axios.get(
          "https://hotelbarvtwo.vercel.app/api/liquorCategory/barMenus"
        );
  ...
  const fetchMenusForCategory = async () => {
    try {
      const response = await axios.get(
        `https://hotelbarvtwo.vercel.app/api/liquorCategory/barMenus/${selectedMainCategory}`
      );
  ...
  const handleDeleteMenus = async () => {
    try {
      ...
      const response = await axios.delete(
        `https://hotelbarvtwo.vercel.app/api/liquorBrand/${selectedMainCategory}/removemenus`,
        {
  ...
  return (
    <>
      <Navbar />
      <div className="container justify-center text-lg mt-2 font-sans font-semibold mx-auto p-5 overflow-x-auto border-gray-300 border-1 max-full"></div>
      <div>
        <div className=" bg-black font-sans">
          {isErrorModalOpen && (
            ...
            >
              <div className="modal-container bg-white w-full md:w-96 p-2 m-4 rounded shadow-lg">
                <div className="text-center">
                  <h3 className="mb-2 text-lg font-semibold text-red-600 dark:text-red-400 mt-1">
                    Selected Menus are already added !
                  </h3>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-4xl mx-auto mt-2 p-4 rounded-md flex flex-col md:flex-row bg-gray-100 font-sans">
        <div className="w-full md:w-1/2 pr-4 mb-4 md:mb-0">
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="mt-1 p-2 w-1/2 border text-sm bg-white rounded-xl focus:outline-none focus:ring focus:border-blue-300 cursor-pointer"
            >
          ...
          <div className="mb-4 overflow-y-auto max-h-96">
              ...
              <>
                <div className="flex items-center mb-2 w-full ">
                  <input
                    ...
                    checked={selectAll}
                    className="mr-2 checkbox text-sm mt-1 cursor-pointer"
                  />
                  <label
                    htmlFor="select-all"
            ...
            {filteredMenus.length > 0 && (
              <button
                ...
                className="bg-red-100 hover:bg-red-200 text-red-700 font-bold py-2 px-4 rounded-lg w-72 mx-auto mt-4"
              >
                <FontAwesomeIcon
                  icon={faTrash}
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="ml-2 text-sm">
              {Object.keys(menuCounts).map((category) => (
                <p className="font-semibold text-green-800" key={category}>Total Menus : {menuCounts[category]}</p>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-between">
            {menus.length > 0 && (
              <button
                ...
                className="bg-orange-200 hover:bg-orange-300 text-orange-900 font-bold py-2 px-4 rounded-lg w-72 mx-auto mt-4"
              >
                <FontAwesomeIcon
                  icon={faCheck}
                  className="cursor-pointer mr-2 text-sm"
      ...
      {isNoMenuSelectedModalOpen && (
        <div
          ...
          style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }}
        >
          <div className="modal-container bg-white w-full md:w-96 p-4 m-4 rounded shadow-lg">
            <div className="text-center">
              <h3 className="mb-1 text-lg font-semibold text-red-600 dark:text-red-400">
                Please Select Brands to add !
              </h3>
            </div>
          </div>
        </div>

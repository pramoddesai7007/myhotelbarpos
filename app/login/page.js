...
export default function Login() {
  ...
  const handleLogin = async () => {
    try {
      ...
      // await axios.post('https://hotelbarvtwo.vercel.app/api/liquorBrand/updateStock');

      // Authenticate the user
      const response = await axios.post('https://hotelbarvtwo.vercel.app/api/auth/login', { username, password });
      ...
      const hotelResponse = await axios.get('https://hotelbarvtwo.vercel.app/api/hotel/get-all');
  ...
  useEffect(() => {
    const loginUser = async () => {
      try {
        ...
        const response = await axios.get("https://hotelbarvtwo.vercel.app/api/section/section/default");
        ...
        <div className="py-12 font-sans">
          <label htmlFor="toggle" className="flex items-center cursor-pointer absolute right-5">
            <div className="relative">
              ...
              <div
                className={`absolute w-6 h-6 bg-blue-300 rounded-full shadow inset-y-0 left-0 ${isOrderEnabled ? "translate-x-full" : "translate-x-0"
                  }`}
              ></div>
            </div>
